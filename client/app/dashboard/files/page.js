...
const FilesPage = () => {
    ...
    useEffect(() => {
        const fetchFiles = async () => {
            const { data, error } = await supabase.storage.from(bucketName).list('files/');
            if (error) {
                ...
                const filesList = data.map(file => ({
                    ...
                    ref: `${supabase.storage.from(bucketName).getPublicUrl('files/' + file.name).publicURL}`
                }));
                setFiles(filesList);
            }
        };
    ...
    const uploadFile = async () => {
        ...
        if (uploadError) {
            ...
            const downloadURL = supabase.storage.from(bucketName).getPublicUrl(filePath).publicURL;
            setFiles(prevFiles => [...prevFiles, { name: file.name, fileId: filePath, dateUploaded: new Date().toLocaleDateString(), ref: downloadURL }]);
        }
    };
    ...
    const deleteFile = async (filePath) => {
        ...
        if (error) {
            ...
            setFiles(files => files.filter(file => 'files/' + file.name !== filePath));
        }
    };
